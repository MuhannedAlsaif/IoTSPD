...
#define SPD_STATUS_IN 21 // GPIO Input
...
#define SurgeADCvalueThreshold 2800  // USING 1100 which is roughly 1v for alpha demo purposes only.  simplify calculations, use adc value for now
...
// HERE  TEST IF STATEMENT this if statement 
 // if( analogRead(ANALOG_ADC1_0_VP) > SurgeADCvalueThreshold ){
   if( analogRead(ANALOG_ADC1_0_VP) > ((110*g_reference_max)/100) ){
    ...
    delay(2); // NO NEED FOR THIS to make the watchdog timer happy
...
void loop() {
 ...
 // digitalWrite(LED_SAG, HIGH);
    ...
    if(digitalRead(SPD_STATUS_IN)) {
      while(digitalRead(SPD_STATUS_IN)){
        Serial.println("DANGER  SPD failure");
        delay(500);
      }
    }
...
//SAG LEVEL -- VERIFY IT         
        if (max_voltage_seen > ((60 *g_reference_max)/100) ) sagLevel = 1; // sag less than 90 by larger than 60
        else if (max_voltage_seen > ((30*g_reference_max)/100) ) sagLevel = 2; // sag less than 60 by larger than 30
        else sagLevel = 3; // sag less than 30 
// --------------
        Serial.print("SAG started at");
      ...
      if(sagFlag){ // sag was happening, and now we just got back regular voltage
        ...
        Serial.print("SAG ended at");
        Serial.println(g_sag_time_end);
//SAG LEVEL -- VERIFY IT 
        if(sagLevel == 1 ) Serial.print("Sag level was 1 ");
        else if (sagLevel == 2 ) Serial.print("Sag level was 2 ");
        else if (sagLevel == 3 )Serial.print("Sag level was 3 ");
        Serial.print("and it lasted ");
        Serial.println((g_sag_time_start - g_sag_time_end));
// --------------
